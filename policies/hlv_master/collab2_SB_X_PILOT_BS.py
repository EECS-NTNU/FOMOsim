    ...
    def get_best_action(self, simul, vehicle):
        ...
        if current_deviation > 0 and next_deviation < 0:
            num_escooters = min(current_deviation, 
                                -next_deviation, 
                                vehicle.bike_inventory_capacity - len(vehicle.get_bike_inventory()) - len(bikes_to_pickup)
                                )
            pickup_escooter_ids, _ = id_escooters_accounted_for_battery_swaps(current_cluster, vehicle, num_escooters, "pickup", self.swap_threshold)
        ...
        return sim.Action(
            ...
            bikes_to_deliver,
            next_location,
            helping_pickup = pickup_escooter_ids,
            helping_delivery = delivery_escooter_ids,
        ...
        for weight_set in self.criticality_weights_set:
            ...
            for plan in plans[max_depth]:
                ...
                if dep_time < end_time:
                    print("end time endres for å håndtere neste besøk")
                    end_time = dep_time + 1
    ...
    def evaluate_route(self, route, scenario_dict, end_time, simul, weights, total_num_bikes_in_system):
        ...
        for visit in route:
            ...
            if net_demand < 0:
                ...
                time_first_violation_no_visit = current_time + min((station.number_of_bikes() - len(station.get_swappable_bikes(BATTERY_LEVEL_LOWER_BOUND)))/ -net_demand, (sum(Ebike.battery for Ebike in sorted_escooters_in_station[-3:])/3)/(calculate_hourly_discharge_rate(simul, total_num_bikes_in_system, False, True)*60))
            else:
            ...
            if net_demand < 0:
                if swap_quantity > loading_quantity+2:
                    time_first_violation_after_visit = eta + min((inventory_after_loading_and_swaps/(-net_demand))*60, 100/calculate_hourly_discharge_rate(simul, total_num_bikes_in_system, False, True))
                else:
                    time_first_violation_after_visit = eta + min((inventory_after_loading_and_swaps/(-net_demand))*60, (sum(ebike.battery for ebike in sorted_escooters_in_station[-3:])/3)/(calculate_hourly_discharge_rate(simul, total_num_bikes_in_system, False, True)*60))
            else:
