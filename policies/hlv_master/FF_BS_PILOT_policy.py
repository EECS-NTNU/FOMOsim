...
class BS_PILOT_FF(Policy):
    ...
    def get_best_action(self, state, vehicle):
        ...
        if isinstance(vehicle.location, sim.Depot): # No action needed if at depot
            ...
            state.metrics.add_aggregate_metric(state, 'depot visits', 1)
        elif vehicle.cluster is None:
            escooters_to_pickup, escooters_to_deliver, batteries_to_swap = calculate_loading_quantities_and_swaps_greedy(vehicle, state, vehicle.location, self.swap_threshold)
            ...
            escooters_to_pickup, escooters_to_deliver, batteries_to_swap = calculate_loading_quantities_and_swaps_greedy(vehicle, state, vehicle.cluster, self.swap_threshold)
            number_of_escooters_pickup = len(escooters_to_pickup)
            number_of_escooters_deliver = len(escooters_to_deliver)
            number_of_batteries_to_swap = len(batteries_to_swap)
        ...
        if vehicle.battery_inventory <= 0 and len(state.depots) > 0 and number_of_batteries_to_swap + number_of_escooters_pickup > 0:
            ...
            escooters_over_target = len(vehicle.get_available_ff_bikes()) - vehicle.cluster.get_target_state(state.day(), state.hour()) + len(escooters_unusable)
        ...
        plan_dict = dict()
        for v in state.get_ff_vehicles():
            # If vehicle is at a location, add current location to the plan with the greedy loading and swap strategy
            if v.eta == 0:
                plan_dict[v.id] = [Visit(v.location, number_of_escooters_pickup, number_of_escooters_deliver, number_of_batteries_to_swap, state.time, v, self.operator_radius)]

            # If the vehicle is driving, use pilot to calculate the loading and swap strategy and add to the plan
            else:
                number_of_escooters_pickup, number_of_escooters_deliver, number_of_batteries_to_swap = self.calculate_loading_quantities_and_swaps_pilot(len(v.bike_inventory), v.battery_inventory, state, v.location, v.eta)
                plan_dict[v.id] = [Visit(v.location, int(number_of_escooters_pickup), int(number_of_escooters_deliver), int(number_of_batteries_to_swap), v.eta, v, self.operator_radius)]
        
        # All locations the vehicles are at or are on their way to is added to the tabu list and plan
        tabu_list = [v.location.id for v in state.get_ff_vehicles()]
        ...
        state.metrics.add_aggregate_metric(state, "accumulated find location time", ending_time - middle_logging_time)
        state.metrics.add_aggregate_metric(state, "accumulated sol time", ending_time - start_logging_time)        
        state.metrics.add_aggregate_metric(state, 'get_best_action', 1)
        state.metrics.add_aggregate_metric(state, 'num battery swaps', len(batteries_to_swap))
        state.metrics.add_aggregate_metric(state, 'num escooter pickups', len(escooters_to_pickup))
        state.metrics.add_aggregate_metric(state, 'num escooter deliveries', len(escooters_to_deliver))
    ...
    def PILOT_function(self, state, vehicle, initial_plan, max_depth, number_of_successors, end_time, total_num_bikes_in_system):
        ...
        """
        hourly_discharge = calculate_hourly_discharge_rate(state, total_num_bikes_in_system, True, False)
        ...
        for weight_set in self.criticality_weights_set:
            ...
            for depth in range(1, max_depth+1):
                ...
                while plans[depth-1] != []:
                    ...
                    if next_vehicle != vehicle:
                        ...
                        new_visits = self.greedy_next_visit(plan, state, num_successors_other_vehicle, weight_set, total_num_bikes_in_system, hourly_discharge)
                    else:
                        new_visits = self.greedy_next_visit(plan, state, number_of_successors, weight_set, total_num_bikes_in_system, hourly_discharge)
            ...
            for plan in plans[max_depth]:
                ...
                while dep_time < end_time:
                    new_visit = self.greedy_next_visit(temp_plan, state, 1, weight_set, total_num_bikes_in_system, hourly_discharge)
    ...
    def calculate_loading_quantities_and_swaps_pilot(self, num_bikes_now, battery_inventory_now, state, cluster, eta):
        """
        ...
        """
        target_state = round(cluster.get_target_state(state.day(),state.hour()))
        time_until_arrival = eta - state.time

        # Calculate number of esooters arriving/leaving in the time period until vehicle arrival
        net_demand = calculate_net_demand(cluster, state.time, state.day(), state.hour(), min(time_until_arrival, 60))
        ...
        for neighbor in cluster.get_neighbours():
            ...
            target_neighbours += round(neighbor.get_target_state(state.day(), state.hour()))
    ...
    def greedy_next_visit(self, plan, state, number_of_successors, weight_set, total_num_bikes_in_system, hourly_discharge):
        """
        ...
        battery_inventory_now = len(vehicle.bike_inventory)
        ...
        for visit in plan.plan[vehicle.id]:
            ...
            battery_inventory_now -= visit.swap_quantity + visit.loading_quantity
        ...
        potential_clusters = find_potential_clusters(state, LOCATION_TYPE_MARGIN, vehicle, num_bikes_now, battery_inventory_now, time_of_departure, plan.plan[vehicle.id][-1].station, self.operator_radius, self.num_clusters)
        ...
        clusters_sorted = calculate_criticality(weight_set, state, potential_clusters, plan.plan[vehicle.id][-1].station, total_num_bikes_in_system ,tabu_list, hourly_discharge)
        ...
        for next_cluster in next_clusters:
            arrival_time = plan.plan[vehicle.id][-1].get_depature_time() + state.get_travel_time(plan.plan[vehicle.id][-1].station.id, next_cluster.id) + MINUTES_CONSTANT_PER_ACTION
            number_of_escooters_to_pickup, number_of_escooters_to_deliver, number_of_escooters_to_swap = self.calculate_loading_quantities_and_swaps_pilot(num_bikes_now, battery_inventory_now, state, next_cluster, arrival_time)
    ...
    def generate_scenarioes(self, state, number_of_scenarios, poisson = True): #normal_dist if poisson = False
        ...
        rng = state.rng
        scenarios = []
        locations_dict = state.locations
        if number_of_scenarios < 1:
            scenario_dict = dict()
            for area_id in locations_dict:
                net_demand =  calculate_net_demand(locations_dict[area_id], state.time ,state.day(),state.hour(), 60)
    ...
    def evaluate_route(self, route, scenario_dict, end_time, state, weights, hourly_discharge):
        ...
        for visit in route:
            ...
            net_demand = scenario_dict[area.id]
            target_state = area.get_target_state(state.day(), state.hour())
            ...
            for neighbor in neighbors:
                ...
                distance_scaling = ((state.get_vehicle_travel_time(area.id, neighbor.id)/60)* VEHICLE_SPEED)/MAX_ROAMING_DISTANCE_SOLUTIONS
    ...
    def return_best_move_average(self, vehicle, state, plan_scores):
        ...
        if list(score_board_sorted.keys())[0] != None:
            ...
            if branch is None: # TODO dette skjer når den ikke klarer å lage plan med neste stopp
                ...
                state.metrics.add_aggregate_metric(state, 'brach None', 1)
                first_move = state.get_closest_depot(vehicle)
                return first_move, None
            first_move = best_plan.plan[vehicle.id][1].station.id
            cluster = best_plan.plan[vehicle.id][1].station
            return first_move, cluster
        
        # If there is no best, choose a random station that is not in the tabu_list
        else: 
            tabu_list = [vehicle2.location.id for vehicle2 in state.get_ff_vehicles()]
            potential_stations2 = [station for station in state.get_areas() if station.id not in tabu_list]    
            rng_balanced = state.rng.default_rng(None)
            cluster = rng_balanced.choice(potential_stations2)
            return cluster.id, cluster

def calculate_loading_quantities_and_swaps_greedy(vehicle, state, location, swap_threshold):
    """
...
def get_max_num_usable_escooters(cluster, vehicle): 
    ...
    """
    return len(cluster.get_available_bikes()) + min(len(cluster.get_unusable_bikes()), vehicle.battery_inventory)
...
def get_escooter_ids_load_swap(cluster, vehicle, num_escooters, target_state, cluster_type, swap_threshold):
    ...
    # Returns lists of escooter IDs on which to load and which to swap batteries on
        ...
        # Number of escooters that can only be picked up, not swapped battery on
        num_escooters_to_only_pickup = round(num_escooters) - num_escooters_to_swap_and_pickup
...
def find_potential_clusters(state, cutoff_vehicle, vehicle, bikes_at_vehicle, batteries_in_vehicle, time_of_departure, departure_location, operator_radius, num_clusters):
    """
    ...
    if cutoff_vehicle * vehicle.bike_inventory_capacity <= bikes_at_vehicle <= (1-cutoff_vehicle)*vehicle.bike_inventory_capacity:
        potential_pickup_stations = find_clusters(areas=state.get_areas(), 
                                                  n=num_clusters, 
