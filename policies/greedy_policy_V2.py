...
class GreedyPolicyV2(Policy):
    ...
    def get_best_action(self, state, vehicle):
        ...
        if ((vehicle.battery_inventory - number_of_batteries_to_swap < vehicle.battery_inventory_capacity * 0.1 )
            ...
            # Always consider both pickup and delivery stations (cutoff filtering removed)
            potential_stations = potential_pickup_stations + potential_delivery_stations

            # Simple candidate scoring: prioritize stations with larger target deviation,
            # then prefer closer stations.
            max_tt = max(list(driving_times.values())) or 1.0
            criticalities = {}
            for station in potential_stations:
                target_state = target_states[station]
                current_bikes = state.locations[station].number_of_bikes()
                deviation = abs(target_state - current_bikes)
                travel_penalty = driving_times[station] / max_tt

                if current_bikes < target_state:
                    actionable_score = deviation if num_bikes_vehicle > 0 else 0.0
                elif current_bikes > target_state:
                    actionable_score = deviation if len(vehicle.get_bike_inventory()) < vehicle.bike_inventory_capacity else 0.0
                else:
                    actionable_score = 0.0

                criticalities[station] = self.omega4 * actionable_score - self.omega3 * travel_penalty
