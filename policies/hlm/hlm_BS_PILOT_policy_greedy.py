...
class Greedy_Policy(Policy): #Add default values from seperate setting sheme
    def __init__(self, 
                max_depth = MAX_DEPTH, 
    ...
    def get_best_action(self, simul, vehicle):
        ...
        number_batteries_to_swap = len(batteries_to_swap)
        ...
        next_station = self.greedy_visit(simul, vehicle, total_num_bikes_in_system, [vehicle.location], self.criticality_weights_set[2])
    ...
    def greedy_visit(self, simul, vehicle, total_num_bikes_in_system, tabu_list, weight_set):
        potential_stations, station_type = find_potential_stations(simul,0.15,0.15,vehicle, len(vehicle.get_bike_inventory()), tabu_list) #Hvor kommer 0.15 fra??
        if potential_stations == []: #Try to find out when and if this happens?
            return None
        
        stations_sorted = calculate_criticality(weight_set, simul, potential_stations, vehicle.location, station_type, total_num_bikes_in_system ,tabu_list)
        stations_sorted_list = list(stations_sorted.keys())

        return stations_sorted_list[0].id
...
def calculate_loading_quantities_and_swaps_greedy(vehicle, simul, station, overflow_criteria, starvation_criteria, upper_threshold):
    ...
    for neighbor in station.neighboring_stations:
        num_escooters_neighbor = neighbor.number_of_bikes() - len(neighbor.get_swappable_bikes(BATTERY_LIMIT_TO_USE))
        neighbor_target_state = round(neighbor.get_target_state(simul.day(), simul.hour()))
    ...
    if num_escooters_accounted_for_battery_swaps < target_state: #Ta hensyn til nabocluster her?
        number_of_escooters_to_deliver = min(len([escooter for escooter in vehicle.get_bike_inventory() if escooter.battery > BATTERY_LIMIT_TO_USE]), target_state - num_escooters_accounted_for_battery_swaps + BIKES_STARVED_NEIGHBOR * starved_neighbors) # discuss 2*starved_neighbors part, ta hensyn til postensielle utladede scootere i bilen
        ...
        number_of_escooters_to_pickup = min(remaining_cap_vehicle, num_escooters_accounted_for_battery_swaps - target_state + BIKES_CONGESTED_NEIGHBOR * overflowing_neighbors, len(station.bikes)) #discuss logic behind this
...
def id_escooters_accounted_for_battery_swaps(station, vehicle, number_of_escooters, station_type, upper_threshold):
    ...
    if station_type == "deliver":
        ...
        number_of_escooters_to_only_swap = min(vehicle.battery_inventory, max(0,len(station.get_swappable_bikes(BATTERY_LIMIT_TO_USE)) - number_of_escooters_to_swap_and_pickup)) if ONLY_SWAP_ALLOWED else 0
