...
# class BS_Greedy(Policy): #Add default values from seperate setting sheme
#     def __init__(self, 
...
#         #############################################################################
#         #  Use Greedy_function to decide next station                                #
#         #############################################################################
...
#         demand = calculate_net_demand(vehicle.location, simul.time, simul.day(), simul.hour(), 60)
#         exp_num = vehicle.location.number_of_bikes() - len(vehicle.location.get_swappable_bikes(20)) + demand
#         criticalities = calculate_criticality(self.criticality_weights_set[0], simul, potential_stations, vehicle.location, calculate_station_type(vehicle.location.get_target_state(simul.day(),simul.hour()), exp_num), total_num_bikes_in_system, tabu_list) # dict {station: criticality} sorted by crit.score                           
...
#             next_station = rng_greedy.choice(potential_stations2).id
#         else: 
#             next_station = list(criticalities.keys())[0].id
...
from policies.hlm.Criticality_score import *
import sim
import time
from settings import *

class BS_Greedy(Policy):
    def __init__(self,crit_weights=[1/6,1/6,1/6,1/6,1/6,1/6], cutoff_vehicle=0.3, cutoff_station=0.15, service_hours=None):
        super().__init__()
        ...
        #- WEIGHTS           
        self.criticality_weights_set = crit_weights

        self.max_depth = None
        self.number_of_successors = None
        self.time_horizon = None
        self.evaluation_weights = None
        self.number_of_scenarios = None
        self.discounting_factor = None
    ...
    def get_best_action(self, simul, vehicle):
        ...
        escooters_to_pickup, escooters_to_deliver, batteries_to_swap = calculate_loading_quantities_and_swaps_greedy(vehicle, simul, vehicle.location, OVERFLOW_CRITERIA, STARVATION_CRITERIA, BATTERY_LEVEL_UPPER_BOUND)

        
        bikes_at_vehicle_after_rebalancing = num_bikes_vehicle + len(escooters_to_pickup) - len(escooters_to_deliver)
        ...
        demand = calculate_net_demand(vehicle.location, simul.time, simul.day(), simul.hour(), 60)
        exp_num = vehicle.location.number_of_bikes() - len(vehicle.location.get_swappable_bikes(20)) + demand
        criticalities = calculate_criticality(self.criticality_weights_set, simul, potential_stations, vehicle.location, calculate_station_type(vehicle.location.get_target_state(simul.day(),simul.hour()), exp_num), total_num_bikes_in_system) # dict {station: criticality} sorted by crit.score                           
