...
class PILOT(Policy):
    ...
    def greedy_next_visit(self, plan, simul, number_of_successors, weight_set):
        ...
        for next_station in next_stations:
            arrival_time = plan.plan[vehicle.id][-1].get_departure_time() + simul.state.traveltime_vehicle_matrix[plan.plan[vehicle.id][-1].station.id][next_station.id] + MINUTES_CONSTANT_PER_ACTION
