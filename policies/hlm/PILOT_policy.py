...
class PILOT(Policy):
    ...
    def get_best_action(self, simul, vehicle):
        ...
        for neighbor in simul.state.stations[next_station].neighboring_stations:
            if neighbor.number_of_bikes() < 0.1*neighbor.get_target_state(simul.day(), simul.hour()) and number_of_bikes_to_deliver>0:
    ...
    def PILOT_function(self, simul, vehicle, initial_plan, max_depth, number_of_successors, end_time):     
        ...
        for weight_set in self.criticality_weights_set:
            ...
            for plan in plans[max_depth]:
                ...
                #     station_list1.append(visit.station.location_id)
                # for visit in plan.plan[1]:
                #     station_list2.append(visit.station.location_id)
    ...
    def greedy_next_visit(self, plan, simul, number_of_successors, weight_set):
        ...
        for next_station in next_stations:
            arrival_time = plan.plan[vehicle.vehicle_id][-1].get_departure_time() + simul.state.traveltime_vehicle_matrix[plan.plan[vehicle.vehicle_id][-1].station.location_id][next_station.location_id] + MINUTES_CONSTANT_PER_ACTION
    ...
    def evaluate_route(self, route, scenario_dict, end_time, simul, weights): #Begins with current station and loading quantities
        ...
        for visit in route:
            ...
            #improved deviation: 
            if net_demand > 0:
                ending_inventory = inventory_after_loading + ((end_time-time)/60)*net_demand
            elif net_demand <= 0:
            ...
            if net_demand > 0:
                ending_inventory_no_visit = initial_inventory+((end_time-current_time)/60)*net_demand
            elif net_demand <= 0:
            ...
            if net_demand > 0:
                excess_bikes_no_visit = initial_inventory+((end_time-current_time)/60)*net_demand
            elif net_demand <= 0:
