...
def run_model(data, roaming=True):
    ...
    if roaming == True:
        ...
        c = m.addVars({(i, t) for i in stations for t in range(1, T_bar+1)},lb=0, vtype=GRB.CONTINUOUS, name="c") #congestions is possible if roaming is False
    ...
    if roaming == True:
        m.addConstrs(l[(i, t-1)] + quicksum(r_L[(j, i, t-T_DC[(j,i)])] for j in possible_previous_stations_cycling[(i,t)]) - quicksum(r_B[(j, i, t-T_DW[(j,i)])] for j in possible_previous_stations_walking[(i,t)]) - quicksum(r_L[(i, j, t)] for j in stations if j != i) + quicksum(r_B[(i, j, t)] for j in neighboring_stations[i]) + D[(i,t)] + quicksum(q_U[(i, t, v)] - q_L[(i, t, v)] for v in vehicles) + s[(i, t)] == l[(i, t)] for t in range(1,T_bar+1) for i in stations)
    else:
        m.addConstrs(l[(i, t-1)] + D[(i,t)] + quicksum(q_U[(i, t, v)] - q_L[(i, t, v)] for v in vehicles) + s[(i, t)] - c[(i, t)] == l[(i, t)] for t in range(1,T_bar+1) for i in stations)
    ...
    if roaming == True:
        ...
        m.setObjective(quicksum(quicksum(W_S*DF[t]*s[(i, t)] + quicksum(W_R*DF[t]*(T_W[(i,j)]/TW_max)*r_B[(i, j, t)] for j in neighboring_stations[i]) + quicksum(W_R*DF[t]*((T_C[(i,j)]+T_W[(i,j)])/TW_max)*r_L[(i, j, t)] for j in stations if j != i) + quicksum(0.01*(q_L[(i, t, v)] +q_U[(i, t, v)]) for v in vehicles)  for t in range(1, T_bar+1))+ W_D*d[i] for i in stations), GRB.MINIMIZE)
