...
def run_model(data):
    ...
    #Variables
    c = m.addVars({(i, t) for i in stations_with_source_sink for t in time_periods},lb=0, vtype=GRB.CONTINUOUS, name="c")
    s = m.addVars({(i, t) for i in stations_with_source_sink for t in time_periods},lb=0, vtype=GRB.CONTINUOUS, name="s")
    d = m.addVars({(i) for i in stations_with_source_sink},lb=0, vtype=GRB.CONTINUOUS, name="d")
    ...
    l = m.addVars({(i, t) for i in stations_with_source_sink for t in time_periods},lb=0, vtype=GRB.CONTINUOUS, name="l") 


    #Constraints
    #Station balance:
    m.addConstrs(l[(i, 0)] == L_0[i] for i in stations_with_source_sink)
    m.addConstrs(l[(i, t-1)]+quicksum(r_L[(j, i, t-T_DC[j][i])]-r_B[(j, i, t-T_DW[j][i])]-r_L[(i, j, t)]+r_B[(i, j, t)] for j in neighboring_stations[i])+D[i][t]+quicksum(q_U[(i, t, v)]-q_L[(i, t, v)] for v in vehicles) + s[(i, t)] - c[(i, t)] == l[(i, t)] for t in range(1,T_bar+1) for i in stations_with_source_sink)
    
    m.addConstrs(l[(i, t)] <= Q_S[i] for i in stations_with_source_sink for t in time_periods)
    ...
    #m.addConstrs(quicksum(q[(j, i, t-T_DD[i][j], v)] for j in stations_with_source_sink) - q_U[(i, t, v)] + q_L[(i, t, v)] == quicksum(q[(i, k, t, v)] for k in stations_with_source_sink) for i in stations for t in range(1, T_bar+1) for v in vehicles)
