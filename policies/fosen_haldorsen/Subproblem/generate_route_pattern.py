        ...
        if not self.starting_station.is_depot():
            if policy == 'greedy':
                ...
                bat_load = max(0, min(starting_station_current_charged_bikes, vehicle_available_bike_capacity,
                                      starting_station_current_charged_bikes - self.starting_station.get_target_state()))
                bat_unload = max(0, min(vehicle_current_charged_bikes, starting_station_available_parking,
                                 self.starting_station.get_target_state() - starting_station_current_charged_bikes))
