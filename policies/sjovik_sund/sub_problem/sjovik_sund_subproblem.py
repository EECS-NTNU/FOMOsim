...
def run_subproblem_model(data):
    ...
    try:
        ...
        T_D  = data["T_D"] # Travel time between stations 
        T_DD = data["T_DD"] # Travel time in periods between stations (time periods)
        T_L  = float(data["T_L"]) # Loading/unloading time per bike (minutes)
        T_M_min = data["T_M_min"] # Minimum maintenance time per station
        T_M_max = data["T_M_max"] # Maximum maintenance time per station
        T_M = data.get("T_M") # Average time to maintain one bike
        Q_V  = data["Q_V"] # Vehicle capacities
        Q_V0 = data["Q_V0"] # Initial vehicle loads
        Q_S  = data["Q_S"] # Station capacities
        I_N0 = data["I_N0"] # Initial station inventories
        I_T  = data["I_T"] # Target station inventories at end of horizon
        D    = data["D"] # Demand at stations over time periods
        w_S, w_C, w_D = data["w_S"], data["w_C"], data["w_D"] # Objective weights
        r_M = data["r_M"] # Maintenance reward weight
        ...
        print(f"[MODEL STATS] Variables: {m.NumVars}, Constraints: {m.NumConstrs}")
