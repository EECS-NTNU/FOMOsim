...
def evaluate_model(
    ...
    # Instantiate the standard MDP config
    mdp_config = MDPConfig.full_maintenance() if ENABLE_COMPONENT_FAILURES else MDPConfig.no_maintenance()

    policy_dict = {
        #"DoNothing": DoNothing(),
        eval_key:    nn_rollout,
        "NN_Greedy": NNGreedyPolicy(nn_model=nn_model, config=mdp_config, depot_id=depot_id),
        "LinearVFA": LinearVFAPolicy(learning_mode=False, maintenance_enabled=ENABLE_COMPONENT_FAILURES),
    }
...
if __name__ == "__main__":
    ...
    rollout.add_argument("--scenarios", type=int, default=3,
                         help="Monte Carlo scenarios per action (default: 3)")
    # --- NEW ARGUMENT ---
    rollout.add_argument("--candidates", type=int, default=5,
                         help="Number of candidates to run full rollout on (default: 5)")
