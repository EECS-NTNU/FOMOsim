...
def _generate_operational_profiles(state, vehicle, maintenance_enabled: bool, verbose: bool):
    ...
    if vehicle.is_at_depot():
        ...
        # Calculate exactly how many broken bikes are currently on the truck
        n_broken_on_vehicle = sum(1 for b in vehicle.get_bike_inventory() 
                                  if getattr(b, 'damage_status', None) in ['depot', 'onsite'])
        
        # Free space will increase once we drop off the broken ones
        effective_free_cap = free_cap + n_broken_on_vehicle
        load_qty = min(repaired_available, effective_free_cap)
        
        profiles.append({
            'rebalancing': 0,
            'onsite_repairs': 0,
            'depot_removals': 0,
            'load_from_queue': load_qty
        })
        if load_qty > 0:
            profiles.append({
                'rebalancing': 0,
...
def generate_candidates(
    ...
    **kwargs,
    ...
    for op in op_profiles:
        ...
        broken_after_op = n_vehicle_broken + op['depot_removals']
