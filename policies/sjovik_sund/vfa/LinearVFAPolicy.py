...
class EpisodeTrainingPolicy(Policy):
    ...
    def __init__(
        ...
        self.midnight_callback = midnight_callback
        self._last_update_day  = -1  # -1 = not yet entered learning phase
    ...
    def get_best_action(self, state, vehicle) -> sim.Action:
        if state.time < self.warmup_end_time:
            ...
            # Detect midnight crossing and trigger batch update + decay step
            if self.midnight_callback is not None:
                current_day = int(state.time / (24 * 60))
                if self._last_update_day < 0:
                    # First learning-phase call — initialise tracker without firing
                    self._last_update_day = current_day
                elif current_day > self._last_update_day:
                    self._last_update_day = current_day
