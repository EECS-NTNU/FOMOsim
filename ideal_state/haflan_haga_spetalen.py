...
def haflan_haga_spetalen_ideal_state(state):
    ...
    compute_and_set_ideal_state(state)
...
def idealize_state(state, day, hour):
    ...
    for cluster in state_rebalanced_ideal_state.stations:
        ...
        positive_deviation = len(cluster.get_available_scooters()) - cluster.get_ideal_state(day, hour)
    ...
    for cluster in state_rebalanced_ideal_state.stations:
        ...
        number_of_scooters_to_add = cluster.get_ideal_state(day, hour) - len(
        ...
        if (percentile - delta) <= 0:
            ...
            for cluster_id in delta_ideal_state_and_outcomes.keys():
                ...
                new_ideal_states[cluster_id] = (
                    state_rebalanced_ideal_state.locations[cluster_id].get_ideal_state(day, hour)
                    + quantile_outcomes
...
def compute_and_set_ideal_state(state):
    ...
    progressbar = Bar("| Computing ideal state", max=7*24)

    for day in range(7):
        for hour in range(24):
            progressbar.next()

            # start by distributing bikes evenly to all stations
            cluster_ideal_states = np.zeros(len(state.locations))
            for st in state.stations:
                cluster_ideal_states[st.id] = num_bikes // num_stations

            normalized_cluster_ideal_states = normalize_to_integers(
                cluster_ideal_states, sum_to=num_bikes
            )

            for cluster in state.stations:
                cluster.ideal_state[day][hour] = normalized_cluster_ideal_states[cluster.id]

            # setting number of scooters to ideal state
            state_rebalanced_ideal_state = idealize_state(state, day, hour)

            # adjusting ideal state by average cluster in- and outflow
            simulate_state_outcomes(state_rebalanced_ideal_state, state, day, hour)
